...
If you cannot bind to port 443 (no sudo, restricted environment), run setup with `--no-bind` instead:

```bash
routeup setup --no-bind --port 8443
```

This installs and trusts the local CA but skips claiming the privileged port. Routes will be served on port 8443 rather than 443.
