...
Omit the flag when no config enables it, or remove/set `public_namespace` to an
empty string in `routeup-server.json`. With it disabled, every public claim
requires a [token](/docs/self-hosting/minting-tokens), and token-less claims are
refused.

## Abuse considerations

With no token to attribute claims to, anonymous limits protect claim creation.
