...
```txt
name          optional default route name (separate from the package's own name)
...
When set, `port_env_var` receives the same value as `PORT`; it cannot name
Routeup-managed variables, `HOST`, `PATH`, or `NODE_EXTRA_CA_CERTS`.

The `routeup.name` is independent of the package's `name` field, the package can
be `example-app-web` while the route is `example-app`. If `routeup.name` is
omitted, bare runner mode uses `ROUTEUP_NAME` and then the working-directory
basename.
